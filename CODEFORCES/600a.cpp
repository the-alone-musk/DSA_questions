#include <iostream>
#include <string>
#include <vector>
using namespace std ; 

bool isInt(string x){

    if(x.size() <= 0 || (x[0] == '0' && x.size() > 1)){return false;}
    for(size_t p = 0; p < x.size(); p++){if(x[p] < '0' || x[p] > '9'){return false;}}
    return true;
}

int main(){

    string s; getline(cin, s);
    const string DELIM = ",;";

    vector<string> integers;
    vector<string> nonint;

    size_t prev(0), pos(0);
    while((pos = s.find_first_of(DELIM, prev)) != string::npos){
        string current = s.substr(prev, pos - prev);
        if(isInt(current)){integers.push_back(current);}
        else{nonint.push_back(current);}
        prev = pos + 1;
    }

    if(prev <= s.size()){
        string current = s.substr(prev, s.size() - prev);
        if(isInt(current)){integers.push_back(current);}
        else{nonint.push_back(current);}
    }

    if(integers.size() > 0){
        cout<< "\"";
        for(size_t p = 0; p < integers.size() - 1; p++){cout << integers[p] << ",";}
        cout << integers.back() << "\"" << endl;
    }
    else{cout << "-" << endl;}

    if(nonint.size() > 0){
        cout<< "\"";
        for(size_t p = 0; p < nonint.size() - 1; p++){cout << nonint[p] << ",";}
        cout << nonint.back() << "\"" << endl;
    }
    else{cout << "-" << endl;}

    return 0;
}